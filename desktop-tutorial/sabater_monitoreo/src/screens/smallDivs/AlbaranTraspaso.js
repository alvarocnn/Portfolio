...
function AlbaranTraspaso(){
    const [AlbaranTraspaso, setAlbaranTraspaso] = useState([]);
    const [isLoading, setLoading] = useState(true);
    useEffect(() => {
        const fetchData = async () => {
            try {
                const responsePending = await fetch('http://localhost:8081/joinAlbaranTraspaso');
                const pendingData = await responsePending.json();
                setAlbaranTraspaso(pendingData);
                setLoading(false); 
    ...
    return(
        ...
        <div className='fondo'>
            <h1 style={{color:"white"}}>Albaran traspaso </h1>
            ...
            {isLoading ? ( 
                ...
                <ul>
                    {AlbaranTraspaso
                        .filter(item => item.Vista === "AlbaranTraspaso")
...
export default AlbaranTraspaso;